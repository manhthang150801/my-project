...
const UpdateTopicPage = () => {
  ...
  const [errors, setErrors] = useState({ topicName: "" });
  ...
  useEffect(() => {
    const fetchTopic = async () => {
      ...
      try {
        ...
        if (data) {
          setFormData({ topicName: data.topicName });
        } else {
          openModal("Lỗi", "Không tìm thấy chủ đề.", true);
        }
      } catch (error) {
  ...
  const validateForm = () => {
    if (!formData.topicName.trim()) {
      setErrors({ ...errors, topicName: "Tên chủ đề không được để trống." });
      return false;
    }
    if (formData.topicName.length > 100) {
      setErrors({ ...errors, topicName: "Tên chủ đề không được dài quá 100 ký tự." });
      return false;
    }
    return true;
  };
    ...
    if (validateForm()) {
      ...
      try {
        ...
        const response = await fetch(`${apiUrl}/topics`, {
          ...
          body: JSON.stringify({ id, topicName: formData.topicName }),
        });
  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-4 bg-gray-100 relative">
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4 text-gray-800 text-center">Cập Nhật Chủ Đề</h2>
        <div>
          <div className="mb-4">
            <label htmlFor="topicName" className="block text-gray-700 font-medium mb-2">
              Tên Chủ Đề
            </label>
            <input
              type="text"
            ...
            />
            {errors.topicName && <p className="text-red-500 text-sm mt-2">{errors.topicName}</p>}
          </div>
