...
const AccountPage = () => {
  ...
  }, [accountIdToDelete, currentPage, pageSize]);

  const fetchAccounts = async (page = 0, size = pageSize) => {
    setIsLoading(true);
    try {
      const token = localStorage.getItem('token');
      const skip = page * size;
      const response = await fetch(`${apiUrl}/users?skip=${skip}&limit=${size}`, {
        method: 'GET',
        headers: {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${apiUrl}/users/${accountIdToDelete}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`, 
          'Content-Type': 'application/json'
      ...
      if (response.ok) {
        openModal('Thành công', 'Xóa tài khoản thành công!', false);
      } else {
        openModal('Lỗi', 'Xóa tài khoản không thành công.', true);
      }
    } catch (error) {
      openModal('Lỗi', 'Có lỗi xảy ra khi xóa tài khoản.', true);
    } finally {
      setIsLoading(false);
    ...
    <div className="flex items-center justify-center min-h-screen p-4 bg-gray-100 relative w-full">
      ...
      <div className="container mx-auto p-1">
        <div className="bg-white p-1 rounded shadow-md w-full">
          <h2 className="text-2xl mb-4 text-gray-800">Danh Sách Tài Khoản</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 overflow-x-auto">
            <table className="min-w-full bg-white">
              <thead>
                <tr>
                  <th className="py-2 px-4 border-b whitespace-nowrap">Username</th>
